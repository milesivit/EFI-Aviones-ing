...
class Plane(models.Model):  # clase avion
    ...
    )  # capacidad. el positive adelante sirve para que no hayan valores negativos, ya que capacidad, fila y columna no tendria que ser negativo
...
class Flight(models.Model):  # clase vuelo
    ...
    )  # duracion, durationfield permite calcular facilmente diferencias entre fechas
...
class Seat(models.Model):
    ...
    seat_type = models.CharField(
        max_length=50
    )  # tipo de asiento, ej: "economico", "business"
...
class Reservation(models.Model):
    ...
    )  # codigo de reserva, unique=True permite que no haya otro codigo igual
